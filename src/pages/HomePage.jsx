...
// ─── Feature colour map ───────────────────────────────────────────────────────
// Maps each color token to the Tailwind classes used in feature cards.
...
// Maps the icon string from data to the actual Lucide component.
const FEATURE_ICON_MAP = { ShieldCheck, Zap, Globe, TrendingUp };

// ─── Sub-components ───────────────────────────────────────────────────────────

/**
 * StatCard — animated counter card that counts up from 0 to `value`
 * once the card enters the viewport.
 */
function StatCard({ value, suffix, label, prefix, delay }) {
  const { count, ref } = useCounter(value);
  return (
    <div ref={ref} className={`reveal feature-card bg-white rounded-2xl p-6 text-center border border-slate-100 shadow-sm ${delay}`}>
      <p className="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-sky-500 to-emerald-500 gradient-text-animate leading-none">
        {prefix}{count}{suffix}
      </p>
      <p className="text-sm text-slate-500 font-medium mt-2">{label}</p>
    </div>
  );
}

/**
 * TestimonialCard — a single quote card with author info and star rating.
 */
function TestimonialCard({ t, delay }) {
  const ref = useReveal(0.1);
  return (
    <article ref={ref} className={`reveal feature-card bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col h-full ${delay}`}>
      <Quote size={28} className="text-sky-200 mb-3 flex-shrink-0" aria-hidden="true" />
      <p className="text-slate-600 text-sm leading-relaxed flex-1 italic">{t.text}</p>
      <div className="flex items-center gap-3 mt-5 pt-4 border-t border-slate-100">
        <img src={t.avatar} alt={t.name} width={40} height={40} className="w-10 h-10 rounded-full object-cover border-2 border-white shadow" />
        <div>
          <p className="text-sm font-bold text-slate-900">{t.name}</p>
          <p className="text-xs text-slate-400">{t.role}</p>
        </div>
        <div className="ml-auto flex gap-0.5" aria-label={`Rating ${t.rating} bintang`}>
...
/**
 * HomePage — the landing experience for NiatBaik.
 *
 * Section order:
 *   1. Hero          — headline, CTA, social proof, carousel
 *   2. Feature Pillars — three trust pillars
 *   3. Impact Stats  — animated counters
 *   4. How It Works  — 3-step explainer + campaign card preview
 *   5. Partner Marquee
 *   6. Testimonials
 *   7. CTA Banner
 */
  ...
  return (
    <main className="overflow-x-hidden">
      ...
      <section
        ...
        className="reveal relative pt-32 pb-20 md:pt-44 md:pb-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto"
      >
        {/* Ambient background blobs */}
        <div className="absolute top-20 left-0 w-72 h-72 bg-sky-100 rounded-full mix-blend-multiply filter blur-3xl opacity-60 animate-blob pointer-events-none" aria-hidden="true" />
        <div className="absolute top-40 right-0 w-72 h-72 bg-emerald-100 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob-delay pointer-events-none" aria-hidden="true" />
        ...
        <div className="relative grid md:grid-cols-2 gap-14 items-center">
          ...
          <div className="space-y-7">
            ...
            <h1 id="hero-heading" className="text-4xl md:text-5xl lg:text-[3.5rem] font-extrabold text-slate-900 leading-[1.15] tracking-tight">
              Wujudkan{' '}
            ...
            <div className="flex items-center gap-5 pt-4 border-t border-slate-100">
              <div className="flex -space-x-3" aria-hidden="true">
                {[11, 12, 13, 14, 15].map((i) => (
                  <img key={i} className="w-9 h-9 rounded-full border-2 border-white object-cover shadow-sm" src={`https://i.pravatar.cc/80?img=${i}`} alt="" />
                ))}
              </div>
              <div>
                <div className="flex items-center gap-1 mb-0.5" aria-label="Rating 4.9 dari 5">
                  {[...Array(5)].map((_, i) => <Star key={i} size={12} className="text-amber-400 fill-amber-400" aria-hidden="true" />)}
      ...
      <section
        ...
        className="reveal py-20 bg-gradient-to-b from-slate-50 to-white"
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {HOME_FEATURES.map((feat, i) => {
              ...
              return (
                <div key={feat.title} className={`feature-card reveal reveal-delay-${i + 1} rounded-2xl p-7 border ${colors.border} ${colors.bg}`}>
                  <div className={`inline-flex p-3 rounded-xl ${colors.icon} mb-5`} aria-hidden="true">
                    <Icon size={26} />
                  </div>
      ...
      <section
        ...
        className="reveal py-16 bg-white border-y border-slate-100"
      >
      ...
      <section
        ...
        className="reveal py-24 bg-white"
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-8">
              <div>
                ...
                <h2 id="how-heading" className="text-3xl md:text-4xl font-extrabold text-slate-900 leading-tight">
              ...
              {[
                ...
                <div key={item.step} className={`reveal reveal-delay-${idx + 1} flex gap-5 items-start`}>
                  <div
                    className="flex-shrink-0 w-11 h-11 rounded-2xl bg-gradient-to-br from-sky-500 to-emerald-500 text-white font-extrabold text-sm flex items-center justify-center shadow-md shadow-sky-500/20"
                    aria-hidden="true"
                  >
                    {item.step}
                  </div>
            ...
            {/* Campaign card preview — desktop only */}
              ...
              <div className="relative bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100">Kesehatan · Aktif</span>
                    <span className="text-xs text-slate-400 font-medium">12 hari lagi</span>
                  </div>
                  <h3 className="font-extrabold text-slate-900 text-sm leading-snug">Bantu Dek Ayu Sembuh dari Leukemia</h3>
                  <div>
      ...
      <section
        ...
        className="reveal py-24 bg-gradient-to-b from-white to-slate-50"
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-xs font-bold text-sky-500 uppercase tracking-widest mb-2">Kata Mereka</p>
            <h2 id="testi-heading" className="text-3xl md:text-4xl font-extrabold text-slate-900">
              Dipercaya oleh Ribuan Orang Baik
            </h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {HOME_TESTIMONIALS.map((t, i) => (
              <TestimonialCard key={t.id} t={t} delay={`reveal-delay-${i + 1}`} />
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ═══════════ 7. CTA BANNER ═══════════ */}
      <section aria-labelledby="cta-heading" className="py-28 relative overflow-hidden">
        {/* Layered background */}
        ...
        />
        {/* Animated blobs */}
        <div className="absolute -top-20 -right-20 w-80 h-80 bg-white/10 animate-blob" aria-hidden="true" />
        <div className="absolute -bottom-20 -left-20 w-96 h-96 bg-emerald-400/20 animate-blob-delay" aria-hidden="true" />

        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/15 text-white text-sm font-semibold mb-6 border border-white/20">
            <Heart size={14} fill="currentColor" aria-hidden="true" />
            Bergabung bersama 100.000+ donatur
          </div>

          <h2 id="cta-heading" className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-6 leading-tight">
          ...
          {/* Trust micro-copy */}
          <div className="flex justify-center gap-8 mt-12 text-white/70 text-xs flex-wrap" role="list">
